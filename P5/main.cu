...
// Define el tipo de valor (float, double, etc.)
...
__global__ void kernel_analysis_L(const int* __restrict__ row_ptr,
    ...
    int lne = threadIdx.x & 0x1f; // identifica el hilo dentro del warp
...
// Definición del struct LevelSizePair
struct LevelSizePair {
    int level;
    int size;

    __host__ __device__
    bool operator<(const LevelSizePair& other) const {
        return (level < other.level) || (level == other.level && size < other.size);
    }
};

// Definición del functor ComputeLevelSize
struct ComputeLevelSize {
    int* row_ptr;
    unsigned int* niveles;

    ComputeLevelSize(int* _row_ptr, unsigned int* _niveles) : row_ptr(_row_ptr), niveles(_niveles) {}

    __host__ __device__
    LevelSizePair operator()(const int& idx) const {
...
// Functor para verificar si el nivel es no vacío
struct IsNonEmptyLevel {
    __host__ __device__
    bool operator()(const LevelSizePair& pair) const {
        return pair.size > 0;
    }
};
...
int ordenar_filas(int* RowPtrL, int* ColIdxL, VALUE_TYPE* Val, int n, int* iorder) {
    ...
    thrust::device_vector<LevelSizePair> d_level_size_pairs(n);

    // Aplicar el functor para calcular las combinaciones nivel-tamaño
    thrust::transform(d_indices.begin(), d_indices.end(), d_level_size_pairs.begin(),
        ComputeLevelSize(RowPtrL, thrust::raw_pointer_cast(d_niveles.data())));
    ...
    //n_warps = thrust::count_if(d_level_size_pairs.begin(), d_level_size_pairs.end(), IsNonEmptyLevel());
...
int main(int argc, char** argv) {
    ...
    // Variables para los eventos de cronometraje
    ...
    std::cout << "Tiempo de ejecución de ordenar_filas: " << milliseconds << " ms" << std::endl;
