...
__global__ void kernel_analysis_L(const int* __restrict__ row_ptr, const int* __restrict__ col_idx,
                                  volatile int* is_solved, int n, unsigned int* niveles) {
...
int ordenar_filas(int* RowPtrL, int* ColIdxL, VALUE_TYPE* Val, int n, int* iorder) {
    thrust::device_vector<unsigned int> d_niveles(n, 0);
    ...
    thrust::host_vector<unsigned int> niveles = d_niveles;

    /* Obtener el máximo nivel */
    unsigned int nLevs = *thrust::max_element(niveles.begin(), niveles.end());

    thrust::host_vector<int> RowPtrL_h(n + 1);
    cudaMemcpy(thrust::raw_pointer_cast(RowPtrL_h.data()), RowPtrL, (n + 1) * sizeof(int), cudaMemcpyDeviceToHost);

    thrust::host_vector<int> ivects(7 * nLevs, 0);
    thrust::host_vector<int> ivect_size(n, 0);
    ...
    // Suma prefija del vector
    thrust::inclusive_scan(ivects.begin(), ivects.end(), ivects.begin());
    ...
    for (int i = 0; i < n; ++i) {
        ...
        iorder[ivects[7 * idepth + vect_size] - 1] = i;
        ivect_size[ivects[7 * idepth + vect_size] - 1] = (vect_size == 6) ? 0 : pow(2, vect_size);
